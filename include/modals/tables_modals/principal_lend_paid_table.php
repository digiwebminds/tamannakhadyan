    ...
    <!-- Remove the nasty inline CSS fixed height on production and replace it with a CSS class — this is just for demonstration purposes! -->
      ...
      if (mysqli_num_rows($result3) > 0) {
        while ($row3 = mysqli_fetch_assoc($result3)) {
          ...
          <td class="p-1 w-1/4 font-bold">Received Principal</td></tr>';
          }elseif($info === 1){
            echo '<tr class="flex w-full mb-4">
            <td class="p-1 w-1/4 font-bold">'.$row3['dorepayment'].'</td>
            <td class="p-1 w-1/4 font-bold">'.$row3['repay_amount'].'</td>
            <td class="p-1 w-1/4 font-bold">Given Principal</td></tr>';
          }else {
            echo '<tr class="flex w-full mb-4">
            <td class="p-1 w-1/4 font-bold">'.$row3['dorepayment'].'</td>
            <td class="p-1 w-1/4 font-bold">'.$row3['repay_amount'].'</td>
            <td class="p-1 w-1/4 font-bold">Late Fine Added</td></tr>';
          }
            }
          }
				echo'</tbody>
